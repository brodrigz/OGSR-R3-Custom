...
{
    static LPCSTR MapLegacyPresetName(LPCSTR args, string32& mapped)
    {
        if (!args || !args[0])
            return args;

        if (!_stricmp(args, "default"))
        {
            xr_strcpy(mapped, "st_opt_dlss_default");
            return mapped;
        }
        ...
        if (args[1] == 0)
        {
            switch (args[0] | 32)
            {
            case 'f': xr_strcpy(mapped, "st_opt_dlss_f"); return mapped;
            case 'j': xr_strcpy(mapped, "st_opt_dlss_j"); return mapped;
            case 'k': xr_strcpy(mapped, "st_opt_dlss_k"); return mapped;
            case 'l': xr_strcpy(mapped, "st_opt_dlss_l"); return mapped;
            case 'm': xr_strcpy(mapped, "st_opt_dlss_m"); return mapped;
            }
        }
...
public:
    ...
    {
        string32 mapped{};
        CCC_Token::Execute(MapLegacyPresetName(args, mapped));
        R_dlss_refresh_available_presets();
