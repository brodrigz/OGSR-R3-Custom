...
void CBlender_BmmD::Compile(CBlender_Compile& C)
{
    ...
    case SE_R2_SHADOW_LIGHTS:
        C.r_Pass("shadow_direct_terrain", RImplementation.o.rsm_enabled ? "shadow_direct_terrain" : "shadow_direct_base", FALSE, TRUE, TRUE, FALSE);
        C.r_dx10Texture("s_base", C.L_textures[0]);
        if (RImplementation.o.rsm_enabled)
        {
            C.r_dx10Texture("s_mask", (std::string(C.L_textures[0].c_str()) + "_mask").c_str());
            C.r_dx10Texture("s_dt_r", oR_Name);
            C.r_dx10Texture("s_dt_g", oG_Name);
            C.r_dx10Texture("s_dt_b", oB_Name);
            C.r_dx10Texture("s_dt_a", oA_Name);
...
void CBlender_BmmD::CompileOLD(CBlender_Compile& C)
{
    ...
    case SE_R2_SHADOW: // smap
    case SE_R2_SHADOW_LIGHTS:
        C.r_Pass("shadow_direct_base", RImplementation.o.rsm_enabled ? "shadow_direct_terrain" : "dumb", FALSE, TRUE, TRUE, FALSE);
        C.r_dx10Texture("s_base", C.L_textures[0]);
        if (RImplementation.o.rsm_enabled)
        {
            C.r_dx10Texture("s_mask", mask);
