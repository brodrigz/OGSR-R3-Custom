...
$arguments = @('/nologo', '/std:c++17', '/EHsc', '/O2', '/MD',
    ...
    "/Fo$outputPath/rsm_gpu_validation.obj", "/Fe$outputPath/rsm_gpu_validation.exe", "$PSScriptRoot/rsm_gpu_validation.cpp",
...
& $compiler @arguments
if ($LASTEXITCODE -ne 0) { throw 'RSM validation executable compilation failed' }
& "$outputPath/rsm_gpu_validation.exe" $outputPath
if ($LASTEXITCODE -ne 0) { throw 'RSM WARP validation failed' }